...
export default async function QualificationScanResultsPage({
  params,
}: {
  ...
  // Get related Qualification for decision makers
  ...
  const [qualificationScan] = qualificationScanId
    ? await db
        .select()
        .from(qualificationScans)
        .where(eq(qualificationScans.id, qualificationScanId))
        .limit(1)
    : [null];

  // Get website audit data
  const [audit] = lead.websiteUrl
    ? await db.select().from(websiteAudits).where(eq(websiteAudits.pitchId, lead.id)).limit(1)
    : [null];
  ...
  const navigationStructure = audit?.navigationStructure
    ? (tryParseJSON(audit.navigationStructure) as
        ...
        | null)
    : null;
  const decisionMakersRaw = qualificationScan?.decisionMakers
    ? (tryParseJSON(qualificationScan.decisionMakers) as
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Qualification Scan Ergebnis</h1>
        </div>
        ...
        <CardContent>
          {decisionMakers && Array.isArray(decisionMakers) && decisionMakers.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">
              Keine Ansprechpartner-Informationen verfügbar. Diese werden während der
              Pre-Qualification-Extraktion ermittelt.
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          {isIbexa ? (
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="mt-4">
                <Button asChild>
                  <Link href={`/pitches/${id}`}>Lead bearbeiten</Link>
                </Button>
              </div>
            </div>
          ) : (
      ...
      <div className="flex gap-2">
        <Button asChild variant="outline">
          <Link href={`/pitches/${id}`}>Zurück zur Übersicht</Link>
        </Button>
        <Button asChild>
          <Link href={`/pitches/${id}/pitch-scan`}>Pitch Scan ansehen</Link>
        </Button>
      </div>
    </div>
  );
