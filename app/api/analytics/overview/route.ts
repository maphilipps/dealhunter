...
export async function GET(request: NextRequest) {
  try {
    ...
    const rfpsWithDecision = allRfps.filter((r) => r.decision !== 'pending' && r.createdAt && r.updatedAt)
    if (rfpsWithDecision.length > 0) {
      const totalTime = rfpsWithDecision.reduce((sum, r) => {
        ...
        const updated = r.updatedAt instanceof Date ? r.updatedAt.getTime() : new Date(r.updatedAt!).getTime()
    ...
    const evaluatingCount = allRfps.filter((r) => ['quick_scanning', 'evaluating', 'bit_pending'].includes(r.status)).length
    ...
    const assignedCount = allRfps.filter((r) => ['team_assigned', 'notified', 'handed_off'].includes(r.status)).length
