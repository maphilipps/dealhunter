...
import { backgroundJobs, pitches, users } from '@/lib/db/schema';

/**
 * GET /api/pitches/[id]/background-job
 *
 * Fetch the latest background job for a lead
 * Used for real-time polling of job progress
 *
 * Query params:
 * - type: 'deep-analysis' | 'pitch' | undefined (filter by job type)
 */
export async function GET(
  ...
  try {
    ...
    // Verify lead exists
    const leads = await db.select().from(pitches).where(eq(pitches.id, leadId)).limit(1);
    ...
    // Search by pitchId (new) or preQualificationId (legacy)
    const whereConditions = or(
      eq(backgroundJobs.pitchId, leadId),
    ...
    const query = db
      ...
      .where(
        jobType
          ? and(whereConditions, eq(backgroundJobs.jobType, jobType as 'deep-analysis' | 'pitch'))
          : whereConditions
      )
    ...
    console.error('[GET /api/pitches/:id/background-job] Error:', error);
