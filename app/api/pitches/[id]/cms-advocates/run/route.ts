 ...
 *
 * POST /api/pitches/[id]/cms-advocates/run
 *
  ...
  try {
    ...
    if (!leadData.preQualificationId) {
      return NextResponse.json(
        { error: 'No Pre-Qualification associated with lead' },
        { status: 400 }
      );
    }
...
function inferCompanySize(
  lead: typeof pitches.$inferSelect
...
function inferBudget(lead: typeof pitches.$inferSelect): 'low' | 'medium' | 'high' {
