...
// ============================================================================
// POST /api/pitches/[id]/pitchdeck/update-team
// ============================================================================
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // 4. Get Lead
    const [lead] = await db.select().from(pitches).where(eq(pitches.id, parsedId.data.id)).limit(1);
    ...
    const [pitchdeck] = await db
      .select()
      .from(pitchdecks)
      .where(eq(pitchdecks.pitchId, parsedId.data.id))
      .limit(1);
    ...
    // 10. Get Pre-Qualification and Employee data for email notifications
      ...
      after(async () => {
        try {
          ...
          if (preQualification.extractedRequirements) {
            try {
              const requirements = JSON.parse(preQualification.extractedRequirements) as Record<
                string,
                unknown
              >;
          ...
          const pitchdeckUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/pitches/${lead.id}/pitchdeck`;
    ...
    console.error('POST /api/pitches/[id]/pitchdeck/update-team error:', error);
    return NextResponse.json(
      {
