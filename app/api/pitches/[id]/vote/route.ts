...
import { pitches, businessUnits, users } from '@/lib/db/schema';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (currentUser.role !== 'bl' && currentUser.role !== 'admin') {
      return NextResponse.json(
        { error: 'Forbidden: Only Business Line Leaders can vote on pitches' },
        { status: 403 }
      );
    }
    ...
    const [updatedLead] = await db
      ...
      .where(eq(pitches.id, parsedId.data.id))
      .returning();
    ...
    console.error('POST /api/pitches/[id]/vote error:', error);
    return NextResponse.json(
      {
