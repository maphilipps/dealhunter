 ...
 * - Returns the latest background job (qualification-scan) for this qualification
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Verify qualification exists and ownership
    ...
    // Get latest qualification job for this qualification
    const jobs = await db
      .select()
      .from(backgroundJobs)
      .where(
