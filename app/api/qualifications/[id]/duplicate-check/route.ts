...
/**
 * POST /api/qualifications/[id]/duplicate-check
 *
...
export async function POST(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    // 2. Fetch Qualification and verify ownership
    ...
    // 3. Check if Qualification has extracted requirements (needed for duplicate check)
    if (!preQualification.extractedRequirements) {
      return NextResponse.json({ error: 'Qualification must be extracted first' }, { status: 400 });
    }

    const extractedReqs = JSON.parse(
    ...
    // 5. Save duplicate check result to Qualification
    await db
