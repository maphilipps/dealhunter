...
import { generateQualificationScanVisualization } from '@/lib/json-render/visualization-agent';
import type { QualificationScanResult } from '@/lib/qualification-scan/agent';

const visualizationRequestSchema = z.object({
  results: z.custom<QualificationScanResult>(data => {
...
/**
 * GET /api/qualifications/[id]/visualization
...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!bid || !bid.qualificationScanId) {
    ...
    if (!qualificationScan?.visualizationTree) {
      return new Response(JSON.stringify({ tree: null }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    ...
    const visualizationTree = JSON.parse(qualificationScan.visualizationTree) as Record<
      string,
      unknown
    >;
...
/**
 * POST /api/qualifications/[id]/visualization
 * Generate and cache a json-render visualization tree from Qualification Scan results
 */
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { results } = parsed.data as { results: QualificationScanResult };
    ...
    const tree = await generateQualificationScanVisualization(results);

    // Cache the visualization in the lead_scans table
