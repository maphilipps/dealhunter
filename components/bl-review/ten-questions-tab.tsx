      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            10 Fragen Bewertung
            <Badge variant={positiveCount >= 7 ? 'default' : positiveCount >= 5 ? 'secondary' : 'destructive'}>
  ...
  return (
    <Card className={statusColors[question.status]}>
      ...
      <CardContent className="space-y-3">
        ...
        {question.details && (
          <p className="text-xs text-muted-foreground">{question.details}</p>
        )}
        <div className="flex items-center gap-2">
...
function extractTenQuestions(
  ...
  const { capabilityMatch, dealQuality, strategicFit, competitionCheck, legalAssessment, referenceMatch, decision } = bitData;
  ...
  return [
    // 1. Tech Stack Match
    {
      ...
      status: capabilityMatch.technologyMatchScore >= 70 ? 'positive' : capabilityMatch.technologyMatchScore >= 50 ? 'warning' : 'negative',
    ...
    // 2. Budget Realistisch
    {
      ...
      icon: DollarSign,
      answer: dealQuality.budgetAdequacy === 'adequate'
      ...
      status: dealQuality.budgetAdequacy === 'adequate' ? 'positive' : dealQuality.budgetAdequacy === 'tight' ? 'warning' : 'negative',
    ...
    // 3. Timeline Machbar
    {
      ...
      icon: Clock,
      answer: dealQuality.timelineRealism === 'realistic'
      ...
      confidence: dealQuality.confidence,
      status: dealQuality.timelineRealism === 'realistic' ? 'positive' : dealQuality.timelineRealism === 'tight' ? 'warning' : 'negative',
    ...
    // 4. Rechtliche Red Flags
    {
      ...
      icon: Scale,
      answer: legalAssessment.criticalBlockers.length === 0
    ...
    // 5. Passende Referenzen
    {
      ...
      status: referenceMatch.overallReferenceScore >= 70 ? 'positive' : referenceMatch.overallReferenceScore >= 50 ? 'warning' : 'negative',
    ...
    // 6. Wettbewerber
    {
      ...
      icon: Users,
      answer: competitionCheck.competitiveAnalysis.knownCompetitors.length > 0
      ...
      status: competitionCheck.competitiveAnalysis.competitionLevel === 'low' || competitionCheck.competitiveAnalysis.competitionLevel === 'none' ? 'positive' : competitionCheck.competitiveAnalysis.competitionLevel === 'medium' ? 'warning' : 'negative',
    ...
    // 7. Win-Wahrscheinlichkeit
    {
      ...
      confidence: competitionCheck.confidence,
      status: competitionCheck.estimatedWinProbability >= 60 ? 'positive' : competitionCheck.estimatedWinProbability >= 40 ? 'warning' : 'negative',
    ...
    // 8. Benötigte Skills
    {
      ...
      confidence: capabilityMatch.confidence,
      status: capabilityMatch.scaleMatchScore >= 70 ? 'positive' : capabilityMatch.scaleMatchScore >= 50 ? 'warning' : 'negative',
    },
    // 9. Skill-Gaps
    {
      id: 9,
      ...
      icon: AlertCircle,
      answer: capabilityMatch.criticalBlockers.length === 0 && capabilityMatch.missingCapabilities.length === 0
      ...
      confidence: capabilityMatch.confidence,
      status: capabilityMatch.criticalBlockers.length === 0 ? (capabilityMatch.missingCapabilities.length === 0 ? 'positive' : 'warning') : 'negative',
      details: capabilityMatch.criticalBlockers.length > 0 ? `Blocker: ${capabilityMatch.criticalBlockers[0]}` : undefined,
    },
...
function getDefaultQuestions(): Question[] {
  ...
  return [
    { id: 1, question: 'Passt der Tech Stack zu unseren Kompetenzen?', icon: Cpu, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 2, question: 'Ist das Budget realistisch?', icon: DollarSign, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 3, question: 'Ist die Timeline machbar?', icon: Clock, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 4, question: 'Gibt es rechtliche Red Flags?', icon: Scale, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 5, question: 'Haben wir passende Referenzen?', icon: Award, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 6, question: 'Wer sind die Wettbewerber?', icon: Users, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 7, question: 'Wie hoch ist die Win-Wahrscheinlichkeit?', icon: TrendingUp, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 8, question: 'Welche Skills werden benötigt?', icon: Briefcase, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 9, question: 'Gibt es Skill-Gaps?', icon: AlertCircle, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
    { id: 10, question: 'Was ist die finale Empfehlung?', icon: ThumbsUp, answer: 'Keine Daten', confidence: 0, status: 'neutral' },
  ];
}
