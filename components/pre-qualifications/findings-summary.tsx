...
export function FindingsSummary({ extractedRequirements, quickScan }: FindingsSummaryProps) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Zusammenfassung aller extrahierten Informationen aus der Pre-Qualification
        </CardDescription>
      </CardHeader>
            ...
            <div className="flex items-start gap-3 rounded-lg bg-muted/50 p-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {req.contacts
                    .filter(c => c.category === 'decision_maker')
                    .length > 0 &&
        ...
        {(req?.technologies?.length || techStack) && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {techStack?.cms && (
                <Badge variant="default">{techStack.cms}</Badge>
              )}
            ...
            <ul className="list-inside list-disc space-y-1 text-sm">
              ...
              {req.keyRequirements.length > 5 && (
                <li className="text-muted-foreground">
                  +{req.keyRequirements.length - 5} weitere
                </li>
              )}
            </ul>
          </div>
          ...
          <div className="rounded-lg border bg-blue-50 p-4">
            <div className="flex items-center justify-between">
              ...
              {quickScan.confidence !== null && quickScan.confidence !== undefined && (
                <Badge variant={quickScan.confidence >= 70 ? 'default' : 'secondary'}>
                  {quickScan.confidence}% Konfidenz
                </Badge>
              )}
            </div>
            {quickScan.reasoning && (
              <p className="mt-2 text-sm text-blue-700">{quickScan.reasoning}</p>
            )}
          </div>
        )}
      </CardContent>
