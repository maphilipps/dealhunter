...
import { rfps, accounts, references, competencies, businessUnits, employees, technologies } from '@/lib/db/schema';
...
export async function buildAgentContext(userId: string): Promise<AgentContext> {
  ...
  const [rfpCountResult] = await db.select({ count: count() }).from(rfps)
    .where(eq(rfps.userId, userId));
  
  const recentRfps = await db.select({
    id: rfps.id,
    status: rfps.status,
  ...
  const allBusinessUnits = await db.select({
    id: businessUnits.id,
    name: businessUnits.name,
  }).from(businessUnits);
  
  const allTechnologies = await db.select({
    id: technologies.id,
    name: technologies.name,
    category: technologies.category,
  }).from(technologies).limit(50);
...
function getPermissionsForRole(role: 'bd' | 'bl' | 'admin'): string[] {
  const basePermissions = [
    'rfp.create', 'rfp.list', 'rfp.get', 'rfp.update',
    'account.create', 'account.list', 'account.get', 'account.update', 'account.delete',
    'reference.create', 'reference.list', 'reference.get',
    'competency.create', 'competency.list', 'competency.get',
  ];
  ...
  if (role === 'admin') {
    return [
      ...
      'technology.create', 'technology.list', 'technology.get', 'technology.update', 'technology.delete',
      'businessUnit.create', 'businessUnit.list', 'businessUnit.get', 'businessUnit.update', 'businessUnit.delete',
      'user.list', 'user.updateRole', 'user.delete',
      'validation.approve', 'validation.reject',
      'reference.update', 'reference.delete',
      'competency.update', 'competency.delete',
    ];
  }
...
export function formatContextForPrompt(context: AgentContext): string {
  ...
  if (context.businessContext) {
    ...
    lines.push(`- Business Units: ${context.businessContext.businessUnits.map(b => b.name).join(', ')}`);
