 ...
 *
 * Provides BU-specific CMS filtering and selection for pitches.
...
import { pitches, technologies, businessUnits } from '@/lib/db/schema';
...
export async function compareCMSOptions(
  ...
  const lead = await db
    .select({ requirements: pitches.requirements, budget: pitches.budget })
    .from(pitches)
    .where(eq(pitches.id, leadId));
