// ═══════════════════════════════════════════════════════════════════════════════
// QUALIFICATIONS SCAN WORKFLOW AGENT - Powered by WorkflowEngine
// New implementation using the Workflow 2.0 architecture
// ═══════════════════════════════════════════════════════════════════════════════

import type { QualificationScanResult } from './agent';
import type {
  TechStack,
  ContentVolume,
  Features,
...
import { qualificationScanSteps } from './workflow/steps';
...
/**
 * Qualification Scan using the new Workflow 2.0 Engine
 *
 ...
 * @example
 * ```ts
 * const result = await runLeadScanWorkflow(
 *   { bidId: '123', websiteUrl: 'https://example.com' },
 *   (event) => console.log(event)
 * );
 * ```
 */
export async function runQualificationScanWorkflow(
  ...
  try {
    emitThought(
      'Qualification Scan Workflow',
      `Starte Qualification Scan Analyse...`,
    ...
    emit({
      ...
      data: {
        agent: 'Qualification Scan Workflow',
    ...
    logActivity('Qualification Scan Workflow fehlgeschlagen', errorMsg);

    emitThought('Error', 'Qualification Scan Workflow Fehler aufgetreten', errorMsg);

    emit({
      type: AgentEventType.ERROR,
      data: {
        message: errorMsg,
        code: 'QUALIFICATION_SCAN_WORKFLOW_ERROR',
      },
    });
    throw error;
  }
...
/** @deprecated Use runQualificationScanWorkflow */
export const runLeadScanWorkflow = runQualificationScanWorkflow;
