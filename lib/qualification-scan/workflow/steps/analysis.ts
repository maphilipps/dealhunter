// ═══════════════════════════════════════════════════════════════════════════════
// ANALYSIS STEPS - QualificationScan 2.0 Workflow
...
export function detectFeatures(websiteData: WebsiteData): Features {
  ...
  return {
    ...
    forms:
      /<form[^>]*>/gi.test(websiteData.html) || /contact|kontakt|newsletter|subscribe/i.test(html),
...
export const playwrightAuditStep = wrapTool<
  { url: string; bidId: string; sitemapUrls?: string[] },
  PlaywrightAuditResult
>(
  {
    ...
    dependencies: ['fetchWebsite'],
    optional: true,
    timeout: 90000, // Increased for multi-page screenshots
  },
  async (input, _ctx) => {
    const { runPlaywrightAudit, identifyKeyPages } = await import('../../tools/playwright');

    // Identify key pages from sitemap for multi-page screenshots
    const keyPageUrls = identifyKeyPages(
      input.url,
      [], // Nav links are detected by playwright itself
      input.sitemapUrls ?? []
    );
