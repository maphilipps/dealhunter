...
/**
 * Reload Timeline Data for Qualification
 *
 * Re-runs the timeline extraction for a specific Qualification.
...
export async function reloadTimeline(preQualificationId: string): Promise<ReloadTimelineResult> {
  try {
    ...
    revalidatePath(`/qualifications/${preQualificationId}/routing`);
...
export async function deletePreQualificationHard(preQualificationId: string) {
  ...
  try {
    ...
    await db.transaction(async tx => {
      ...
      await tx.delete(leadScans).where(eq(leadScans.preQualificationId, preQualificationId));
      await tx
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Fehler beim Löschen des Qualifications',
    };
  }
}
