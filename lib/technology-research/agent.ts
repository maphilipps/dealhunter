...
export async function runTechnologyResearch(
  ...
  try {
    ...
    const isStale = input.existingData?.lastResearchedAt
      ? Date.now() - new Date(input.existingData.lastResearchedAt).getTime() >
        30 * 24 * 60 * 60 * 1000
      : true;
...
- License: ${repoInfo.license || 'N/A'}`;
            ...
            logActivity(
              'GitHub Research',
              `Found: v${repoInfo.latestVersion}, ${repoInfo.githubStars} stars`
            );
          }
        }
        ...
        logActivity(
          'Research Warning',
          `Intelligent research failed: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
    ...
    logActivity(
      'Researching technology information',
      `Fields to fill: ${emptyFields.length > 0 ? emptyFields.join(', ') : 'refreshing stale data'}`
    );
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a technology researcher. Always respond with valid JSON matching the requested schema. Do not include markdown code blocks or any other formatting - just the raw JSON object.',
        },
        {
          role: 'user',
          content:
    ...
    if (Object.keys(githubData).length > 0) {
      logActivity(
        'Merging GitHub data',
        `Adding ${Object.keys(githubData).length} fields from GitHub API`
      );
      parsedResult = {
        ...parsedResult,
        ...
        ...(!parsedResult.description &&
          githubData.description && { description: githubData.description }),
      };
    }
    ...
    logActivity(
      'Technology Research completed successfully',
      `Updated fields: ${Object.keys(filteredResult).length}`
    );
...
function buildResearchPrompt(
  ...
  const fieldDescriptions: Record<string, string> = {
    ...
    targetAudiences:
      'Target audiences for this technology (e.g., enterprise, startups, government)',
    useCases: 'Common use cases and scenarios',
    adessoExpertise:
      'How adesso SE (a leading IT consulting company) could position expertise with this technology',
  };
...
function filterNewFields(
  ...
  if (!existingData.latestVersion && result.latestVersion)
    filtered.latestVersion = result.latestVersion;
  ...
  if (!existingData.communitySize && result.communitySize)
    filtered.communitySize = result.communitySize;
  ...
  if (!existingData.targetAudiences && result.targetAudiences)
    filtered.targetAudiences = result.targetAudiences;
  if (!existingData.useCases && result.useCases) filtered.useCases = result.useCases;
  if (!existingData.adessoExpertise && result.adessoExpertise)
    filtered.adessoExpertise = result.adessoExpertise;
