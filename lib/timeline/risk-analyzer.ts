 ...
 *
 * Based on delta = Qualification_Deadline - AI_Estimate:
 * - HIGH: delta < -30 days (Qualification wants it >20% faster than realistic)
 ...
 *
 * @param rfpDeadline - Qualification deadline (ISO string or Date)
...
export function analyzeTimelineRisk(
  ...
  // Handle missing Qualification deadline
  if (!rfpDeadline) {
    return {
      risk: 'LOW',
      deltaDays: 0,
      warning: 'Keine Qualification-Deadline angegeben - keine Risiko-Analyse möglich',
      rfpDeadline: null,
  ...
  // If Qualification is before AI completion, delta is negative (bad)
  // If Qualification is after AI completion, delta is positive (good)
  let deltaDays: number;
  if (rfpDate < aiCompletionDate) {
    // Qualification deadline is BEFORE AI estimate → negative delta
    deltaDays = -calculateWorkingDays(rfpDate, aiCompletionDate);
  } else {
    // Qualification deadline is AFTER AI estimate → positive delta (buffer)
  ...
  if (deltaDays < RISK_THRESHOLDS.HIGH_THRESHOLD) {
    // HIGH RISK: Qualification deadline is >30 working days (6+ weeks) before AI estimate
    risk = 'HIGH';
    isRealistic = false;
    ...
    warning = `Unrealistische Timeline! Qualification fordert ${rfpMonths} Monate, AI schätzt ${aiMonths} Monate. Das Projekt müsste ${shortfallWeeks} Wochen früher fertig sein als realistisch.`;
  } else if (deltaDays < RISK_THRESHOLDS.MEDIUM_THRESHOLD) {
    // MEDIUM RISK: Qualification deadline is 0-30 days before AI estimate
    risk = 'MEDIUM';
    isRealistic = false;
    ...
    // LOW RISK: Qualification deadline is after AI estimate (has buffer)
    risk = 'LOW';
    isRealistic = true;

    const bufferWeeks = Math.round(deltaDays / 5);
    warning = `Qualification-Deadline realistisch - ${bufferWeeks} Wochen Buffer vorhanden.`;
  }
