...
When working on UI changes, verify the application visually:

1. Start the dev server if not already running (`d3k`)
2. Navigate to the relevant page and take screenshots to verify the UI looks correct
3. Test interactive elements (buttons, forms, navigation) work as expected

Use one of the following tools for browser interaction:

- **agent-browser** (headless CLI): `agent-browser --profile ~/.dealhunter-profile open <url>`
- **Claude in Chrome** (MCP): use `mcp__claude-in-chrome__*` tools if the Chrome extension is available

If the dev server or the app has issues, run `fix my app` to diagnose and resolve problems.
